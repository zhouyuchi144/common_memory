...
system_prompt = """# 任务
你是一个严谨的信息提取助手。从用户多轮对话中识别并提取用户明确提及或确认的【家的地址】和【公司地址】

# 提取要求
1.明确归属：
- 家的地址 (home): 必须与 "家"、"回家"、"我住的"、"小区"、"住址" 等词语直接绑定，或紧邻的上下文中直接关联
- 公司地址 (comp)：必须与 "公司"、"上班"、"单位"、"办公室" 等词语直接绑定，或紧邻的上下文中直接关联
2.禁止进行任何联想推断。例如，**绝对不能**因为行程的终点是“家”，就自动推断其起点是“公司”
- 示例：用户说“从昊海大厦地出发回家”，昊海大厦不是“公司”
3.排除无关地点：对话中仅作为临时出发地、目的地、途经点提到的地址，若没有上述明确的归属词语关联，则**一律忽略**，不得提取。
4.**完整性**：提取的地址应包含所有细节。

# 严格按照以下步骤执行
## 第1步：对每行【多轮对话】数据，提取【家的地址】和【公司地址】
## 第2步：合并结果
...
示例：{"home": "菊园2号楼4单元301;知春路(公交站)", "comp": "昊海大厦(东南门)"}"""
...
def main(current_date):
    ...
    grouped_df = filtered_df.groupBy("user_id", "intent_id") \
        .agg(collect_list(struct(col("role").alias("role"), col("msg").alias("msg"), col("conversation_id").alias("conversation_id"))).alias("messages"))
